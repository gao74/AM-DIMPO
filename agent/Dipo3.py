...
class DiPo(object):
    ...
    def append_memory(self, state, action, reward, next_state, mask):
        ...
        priority = 1.0  # 初始优先级
        self.memory.append(state, action, reward, next_state, mask, priority)
    ...
    #         grad_norm = torch.norm(best_actions.grad[i])
    #         grad_norms.append(grad_norm.item())
    #
    #     # 更新经验池中的优先级
    #     self.memory.update_priorities(idxs, np.array(grad_norms))
    #
    ...
    def train(self, iterations, batch_size=256, log_writer=None):
        for _ in range(iterations):
            states, actions, rewards, next_states, masks, idxs = self.memory.sample(batch_size)
